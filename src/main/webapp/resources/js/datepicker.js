...
function fznDatePicker ($log, $document, $filter) {
    ...
    var partials = {
            ...
            '</tr>'+
            '</thead>',
            footTemplateDays: '<tfoot class="picker {{todayClass}}" ng-show="todayButton"><tr><th colspan="7" class="today">초기화</th></tr></tfoot>'
            	/*'<th class="prev"><i class="material-icons">chevron_left</i></th>'+*/
            	/*'<th class="next"><i class="material-icons">chevron_right</i></th>'+*/
    };
    ...
    var template = '<div class="fzn-date-picker"> ' +
        ...
        '<input readonly id={{pickerid}} class="date-input" placeholder="{{placeholder}}" value="{{modelviewvalue}}" style="text-align:center;margin:0px;">' +
        '<span class="date-input-icon"><button class="btn_calendar" type="button" ng-click="displayPicker()"></button></span>' +
...
//            debugger;
        	var rect = element.children('.date-display')[0].getBoundingClientRect();
//        	console.log(element.children('.date-display')[0].offsetTop);
//        	console.log(element.children('.date-display')[0].offsetLeft);
            scope.picker.css({
            	width:"250px",
                top:    element.children('.date-display')[0].offsetTop + element.children('.date-display')[0].clientHeight + 'px',
//            	top:    rect.top + rect.height + 'px',
                left:   element.children('.date-display')[0].offsetLeft + 'px',
                zIndex: 2048,
                display: "block",
                position: "absolute"
            });
...
//            debugger;
        ...
        scope.init = function () {
            ...
            scope.todayButton       = attrs.today       || true;
